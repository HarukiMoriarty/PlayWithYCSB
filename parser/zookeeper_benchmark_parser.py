...
def parse_zookeeper_result(file_path):
    """Parse a ZooKeeper YCSB benchmark result file and extract key metrics."""
    metrics = {}
    ...
    with open(file_path, 'r') as f:
        ...
        match = re.search(r'zookeeper_workload([a-f])_(\d+)threads\.txt', filename)
        if match:
...
def collect_all_results(results_dir):
    ...
    # Find all ZooKeeper result files
    result_files = list(Path(results_dir).glob('zookeeper_workload*_*threads.txt'))
    
    for file_path in result_files:
        try:
            metrics = parse_zookeeper_result(file_path)
            if metrics:
...
def create_throughput_by_workload_chart(results, output_dir):
    ...
    plt.title('ZooKeeper YCSB Throughput by Workload and Thread Count', fontsize=14)
    ...
    # Save the figure
    output_path = os.path.join(output_dir, 'zookeeper_throughput_by_workload.png')
...
def create_latency_by_thread_chart(results, output_dir):
    ...
    plt.title('ZooKeeper YCSB Average Read Latency by Workload and Thread Count', fontsize=14)
    ...
    # Save the figure
    output_path = os.path.join(output_dir, 'zookeeper_avg_latency_by_workload.png')
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
...
def create_p99_latency_chart(results, output_dir):
    ...
    plt.title('ZooKeeper YCSB P99 Read Latency by Workload and Thread Count', fontsize=14)
    ...
    # Save the figure
    output_path = os.path.join(output_dir, 'zookeeper_p99_latency_by_workload.png')
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
...
def main():
    parser = argparse.ArgumentParser(description='Parse ZooKeeper YCSB benchmark results and generate visualizations')
    parser.add_argument('--results-dir', type=str, default='results/zookeeper', 
                        help='Directory containing ZooKeeper YCSB benchmark result files')
