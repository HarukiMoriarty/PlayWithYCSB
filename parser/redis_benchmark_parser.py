...
def parse_redis_result(file_path):
    """Parse a Redis YCSB benchmark result file and extract key metrics."""
    metrics = {}
    ...
    with open(file_path, 'r') as f:
        ...
        match = re.search(r'redis_workload([a-f])_(\d+)threads\.txt', filename)
        if match:
...
def collect_all_results(results_dir):
    ...
    # Find all Redis result files
    result_files = list(Path(results_dir).glob('redis_workload*_*threads.txt'))
    
    for file_path in result_files:
        try:
            metrics = parse_redis_result(file_path)
            if metrics:
...
def create_throughput_by_workload_chart(results, output_dir):
    ...
    plt.title('Redis YCSB Throughput by Workload and Thread Count', fontsize=14)
    ...
    # Save the figure
    output_path = os.path.join(output_dir, 'redis_throughput_by_workload.png')
...
def create_latency_by_thread_chart(results, output_dir):
    ...
    plt.title('Redis YCSB Average Read Latency by Workload and Thread Count', fontsize=14)
    ...
    # Save the figure
    output_path = os.path.join(output_dir, 'redis_avg_latency_by_workload.png')
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
...
def create_p99_latency_chart(results, output_dir):
    ...
    plt.title('Redis YCSB P99 Read Latency by Workload and Thread Count', fontsize=14)
    ...
    # Save the figure
    output_path = os.path.join(output_dir, 'redis_p99_latency_by_workload.png')
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
...
def main():
    parser = argparse.ArgumentParser(description='Parse Redis YCSB benchmark results and generate visualizations')
    parser.add_argument('--results-dir', type=str, default='results/redis', 
